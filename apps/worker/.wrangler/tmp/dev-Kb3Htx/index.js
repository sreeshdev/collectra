...
// .wrangler/tmp/bundle-nXriZQ/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-nXriZQ/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
function isDbConnectionError(err2) {
  const msg = err2 instanceof Error ? err2.message : String(err2);
  return DB_CONNECTION_ERROR_PATTERNS.some((p) => p.test(msg));
}
__name(isDbConnectionError, "isDbConnectionError");
var DB_UNAVAILABLE_MESSAGE = "Database is temporarily unavailable (waking from sleep). Please try again in a few seconds.";
var DEFAULT_OPTIONS = {
  maxRetries: 2,
  // 3 total attempts
  retryDelayMs: 1500
...
async function withDbRetry(fn, options = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = err2;
      if (!isDbConnectionError(err2) || attempt === maxRetries) {
        throw err2;
      }
...
__name(withDbRetry, "withDbRetry");
function sleep(ms) {
...
auth.post("/login", async (c) => {
  try {
    ...
    const message = isDbConnectionError(error3) ? DB_UNAVAILABLE_MESSAGE : error3.message || "Login failed";
...
auth.get("/me", authMiddleware, async (c) => {
  try {
    ...
    const message = isDbConnectionError(error3) ? DB_UNAVAILABLE_MESSAGE : error3.message || "Failed to fetch user";
    return c.json({ error: message }, 500);
  }
});
auth.post("/change-password", authMiddleware, async (c) => {
  try {
    ...
    const message = isDbConnectionError(error3) ? DB_UNAVAILABLE_MESSAGE : error3.message || "Failed to change password";
    return c.json({ error: message }, 500);
  }
});
...
var resendSchema = external_exports.object({
  transactionIds: external_exports.array(external_exports.string().min(1)).min(1).max(50)
  // Razorpay plink IDs (plink_xxx)
});
var payments = new Hono2().basePath("/payments");
var RESEND_BATCH_SIZE = 20;
...
var BULK_MAX_CUSTOMERS_PER_REQUEST = 15;
...
payments.post("/resend", authMiddleware, async (c) => {
  try {
    ...
    const auth2 = btoa(`${c.env.RAZORPAY_KEY_ID}:${c.env.RAZORPAY_KEY_SECRET}`);
    let sent = 0;
    let failed = 0;
    ...
    return c.json({ error: error3?.message || "Resend failed" }, 500);
  }
});
...
webhooks.post("/razorpay", async (c) => {
  try {
    ...
    const isValid2 = await verifyRazorpaySignature(
...
app.onError((err2, c) => {
  try {
    console.error(err2);
    const message = isDbConnectionError(err2) ? DB_UNAVAILABLE_MESSAGE : "Internal Server Error";
...
function isConnectionTimeout(err2) {
  return isDbConnectionError(err2);
}
__name(isConnectionTimeout, "isConnectionTimeout");
var REQUEST_TIMEOUT_MS = 28e3;
...
var src_default = {
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      const isTimeout = err2 instanceof Error && (err2.message === "REQUEST_TIMEOUT" || isConnectionTimeout(err2));
      try {
        return buildCorsErrorResponse(
          request,
          env2,
          isTimeout ? DB_UNAVAILABLE_MESSAGE : void 0
        );
      } catch (finalErr) {
        console.error(
...
var RESEND_CRON_DAYS = [6, 11, 16, 21, 26];
var RESEND_BATCH_SIZE2 = 8;
var RESEND_MEDIUM = "sms";
async function handleCron(env2) {
  ...
  try {
    ...
    await resendPendingPaymentLinks(prisma, env2, today);
    if (currentDay === 1) {
      const customers2 = await prisma.customer.findMany({
        include: { package: true }
      });
      for (const customer of customers2) {
        try {
          ...
          console.error(`Billing error for ${customer.id}:`, error3);
        }
      }
    }
...
async function resendPendingPaymentLinks(prisma, env2, today) {
  ...
  const skip2 = batchIndex * RESEND_BATCH_SIZE2;
  const pending = await prisma.transaction.findMany({
    where: {
      ...
      transactionId: { not: "" }
      // Must have Razorpay plink ID
    },
    select: { id: true, transactionId: true },
    orderBy: { createdAt: "asc" },
    skip: skip2,
    take: RESEND_BATCH_SIZE2
  });
  if (pending.length === 0) {
    console.log("Resend: No pending payment links to resend");
    return;
  }
  const auth2 = btoa(`${env2.RAZORPAY_KEY_ID}:${env2.RAZORPAY_KEY_SECRET}`);
  let success = 0;
  let failed = 0;
  for (const tx of pending) {
    try {
      ...
      if (res.ok) {
        ...
        console.error(`Resend: ${tx.transactionId} failed:`, err2);
      }
    } catch (e) {
      failed++;
...
__name(resendPendingPaymentLinks, "resendPendingPaymentLinks");
